@php
    ...
    $queryParams = collect(request()->query())->except(['lang', 'locale'])->all();
@endphp
<!DOCTYPE html>
        ...
        <header class="site-header">
            <div class="container header-row">
                ...
                <nav class="desktop-nav" aria-label="Primary">
                    <a href="{{ $anchorBase }}#services">{{ $landing['nav']['services'] }}</a>
                    <a href="{{ $anchorBase }}#stack">{{ $landing['nav']['stack'] }}</a>
                    <a href="{{ $anchorBase }}#projects">{{ $landing['nav']['projects'] }}</a>
                    <a href="{{ $anchorBase }}#proof">{{ $landing['nav']['proof'] }}</a>
                    <a href="{{ $anchorBase }}#process">{{ $landing['nav']['process'] }}</a>
                    <a href="{{ route('contact.create', ['locale' => $locale]) }}">{{ $landing['nav']['contact'] }}</a>
                </nav>

                <div class="header-actions">
                    <form method="GET" action="{{ url()->current() }}" class="locale-form header-locale">
                        @foreach($queryParams as $name => $value)
                            <input type="hidden" name="{{ $name }}" value="{{ $value }}">
                        @endforeach
                        <label class="sr-only" for="locale-select">{{ $landing['nav']['language'] }}</label>
                        <select id="locale-select" name="locale" data-lang-select aria-label="{{ $landing['nav']['language'] }}">
                            @foreach($localeLabels as $supportedLocale => $label)
                                <option value="{{ $supportedLocale }}" @selected($locale === $supportedLocale)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </form>
                    ...
                    <a href="{{ $anchorBase }}#contact" class="btn btn-primary header-cta">{{ $landing['nav']['start_project'] }}</a>
                </div>
            </div>
            ...
            <details class="mobile-nav">
                ...
                <div class="mobile-nav-panel">
                    ...
                    <a href="{{ $anchorBase }}#process">{{ $landing['nav']['process'] }}</a>
                    <a href="{{ route('contact.create', ['locale' => $locale]) }}">{{ $landing['nav']['contact'] }}</a>
                    <form method="GET" action="{{ url()->current() }}" class="locale-form mobile-locale-form">
                        @foreach($queryParams as $name => $value)
                            <input type="hidden" name="{{ $name }}" value="{{ $value }}">
                        @endforeach
                        <label class="sr-only" for="mobile-locale-select">{{ $landing['nav']['language'] }}</label>
                        <select id="mobile-locale-select" name="locale" data-lang-select aria-label="{{ $landing['nav']['language'] }}">
                            @foreach($localeLabels as $supportedLocale => $label)
                                <option value="{{ $supportedLocale }}" @selected($locale === $supportedLocale)>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <a href="{{ $anchorBase }}#contact" class="btn btn-primary">{{ $landing['nav']['start_project'] }}</a>
                </div>
            </details>
        </header>
        ...
        <footer class="site-footer">
            <div class="container footer-grid">
                ...
                <div class="footer-column">
                    ...
                    <a href="{{ $anchorBase }}#services">{{ $landing['nav']['services'] }}</a>
                    <a href="{{ $anchorBase }}#stack">{{ $landing['nav']['tech_stack'] }}</a>
                    <a href="{{ $anchorBase }}#projects">{{ $landing['nav']['projects'] }}</a>
                    <a href="{{ $anchorBase }}#process">{{ $landing['nav']['process'] }}</a>
                </div>
                <div class="footer-column">
                    ...
                    <div class="footer-locales">
                        @foreach($localeLabels as $supportedLocale => $label)
                            <a href="{{ route(\Illuminate\Support\Facades\Route::currentRouteName() ?? 'home', array_filter(array_merge(request()->route()?->parameters() ?? [], ['locale' => $supportedLocale]))) }}">{{ $label }}</a>
                        @endforeach
                    </div>
                </div>
            </div>
